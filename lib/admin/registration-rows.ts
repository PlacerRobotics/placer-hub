// Shared row-building for the admin registrations views — one query set, two
// consumers: /admin/registrations (V5/Combat MS/HS) and /admin/registrations-iq
// (VEX IQ). Each page filters allRows/allTeams by program itself; this only
// gathers the unfiltered universe so the two pages can't drift out of sync.
import type { RegRow, TeamOpt } from '@/app/admin/registrations/registrations-manager'

const SEASON = '2026-27'

export async function gatherAllRegistrationRows(supabase: any): Promise<{ allRows: RegRow[]; allTeams: any[] }> {
  ...
  return { allRows, allTeams: (allTeams ?? []) as any[] }
}

export function toTeamOpts(allTeams: any[], filterProgram?: (program: string) => boolean): TeamOpt[] {
  return allTeams
    .filter((t: any) => (filterProgram ? filterProgram(t.program) : true))
    .map((t: any) => ({ id: t.id, label: `${t.team_number ?? t.team_name ?? t.id} · ${t.program} · ${t.division}` }))
}
