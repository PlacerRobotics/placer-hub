...
// family. No magic link here — invites go out on approval.
async function addMemberStub(db: any, familyId: string, sFirst: string, sLast: string, teamId: string) {
  ...
  if (!stu) {
    const { data: s, error } = await db.from('student').insert({ family_id: familyId, first_name: sFirst, last_name: sLast, city: 'Unknown', zip_code: '00000', grade: 0, status: 'active' }).select('id').single()
...
export async function POST(req: NextRequest) {
  ...
  for (const r of others) {
    ...
    if (isCoachChild) {
      const res = await addMemberStub(db, coachFamilyId, sFirst, sLast, teamId)
