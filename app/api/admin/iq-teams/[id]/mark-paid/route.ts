...
export async function POST(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: team } = await db.from('team').select('id, program, status, team_fee_amount, team_payment_reference_code').eq('id', teamId).maybeSingle()
  if (!team || team.program !== 'vex_iq') return NextResponse.json({ error: 'IQ team not found.' }, { status: 404 })
  if (team.status !== 'pending_payment') return NextResponse.json({ error: 'This team is not awaiting payment.' }, { status: 400 })
  ...
  await db.from('team').update({ team_fee_status: 'paid', status: 'pending_admin_confirmation' }).eq('id', teamId)
