...
type Guardian1 = { id: string; first_name: string; last_name: string; login_email: string; communication_email: string; phone: string }
...
export default function FamilyActions({ familyId, guardian1, students }: { familyId: string; guardian1: Guardian1 | null; students: Student[] }) {
  ...
  const [editG1, setEditG1] = useState(false)
  ...
  return (
    <div style={card}>
      ...
      <div style={{ display: 'flex', gap: '0.625rem', flexWrap: 'wrap', alignItems: 'center' }}>
        <button type="button" style={btn} disabled={!!busy} onClick={() => setShowEmail((v) => !v)}>Change login email</button>
      ...
      {showEmail && (
        <div style={{ marginTop: '1rem', display: 'flex', gap: '0.5rem', alignItems: 'flex-end', flexWrap: 'wrap' }}>
          <div style={{ flex: 1, minWidth: 240 }}>
            ...
            <input style={input} type="email" value={newEmail} onChange={(e) => setNewEmail(e.target.value)} placeholder={guardian1?.login_email} />
          </div>
          <button type="button" style={navyBtn} disabled={busy === 'email' || !newEmail.trim()} onClick={async () => {
            const d = await call('email', `/api/admin/families/${familyId}/change-email`, { new_email: newEmail.trim() })
            if (d) { setActionMsg(d.authUpdated ? 'saved' : 'saved (guardian only — no auth user yet)'); setShowEmail(false); setNewEmail(''); router.refresh() }
          }}>Update email</button>
        </div>
      )}
      {viewLink && (
      ...
      {guardian1 && (
        <div style={{ marginTop: '1.25rem', borderTop: '1px solid var(--color-border)', paddingTop: '1rem' }}>
          <button type="button" style={btn} onClick={() => setEditG1((v) => !v)}>{editG1 ? 'Cancel' : 'Edit guardian 1 contact'}</button>
          {editG1 && <GuardianEdit familyId={familyId} g={guardian1} onDone={() => { setEditG1(false); router.refresh() }} />}
        </div>
      )}
...
function GuardianEdit({ familyId, g, onDone }: { familyId: string; g: Guardian1; onDone: () => void }) {
