  ...
  return (
    <>
      ...
      <p style={{ fontSize: '0.875rem', color: 'var(--color-text-muted)', margin: '0 0 1rem' }}>
        Read and accept each agreement below, then sign once at the bottom.
      </p>
      ...
      {toSign.map((w) => (
        <div key={w.id} style={{ backgroundColor: 'var(--color-surface)', border: '1px solid var(--color-border)', borderRadius: 10, padding: '1.25rem', marginBottom: '1.25rem' }}>
          ...
          <details open style={{ border: '1px solid var(--color-border)', borderRadius: 6, padding: '0.875rem', marginBottom: '0.875rem' }}>
            <summary style={{ cursor: 'pointer', fontSize: '0.8125rem', fontWeight: 600, color: 'var(--color-navy-deep)' }}>Full agreement — read below, then collapse if you like</summary>
            <div style={{ fontSize: '0.875rem', lineHeight: 1.7, color: 'var(--color-text-primary)', whiteSpace: 'pre-wrap', marginTop: '0.875rem' }}>{w.body_markdown}</div>
          </details>
